...
def menu_selector(menu_items):
    ...
    def display_order_list(order_counts):
        ...
        while True:
            ...
            clear_screen()
            lcd.print_line("Your Current Order:")
    ...
    while True:
        ...
        if GPIO.input(UP_BUTTON) == GPIO.LOW:
            ...
            order_counts[menu_items[current_index+selected_index]] += 1  # Increment the count for the selected item
            clear_screen()
            lcd.print_line(f"Added {menu_items[current_index+selected_index]}",1)
            time.sleep(1)
        elif GPIO.input(VIEW_BUTTON) == GPIO.LOW:
