...
if st.session_state.cam_active:
    with st.expander("Petunjuk Penggunaan"):
        ...
        **Langkah-langkah:**
        ...
        5. Tekan "Stop" untuk menghentikan, atau "Reset Proses" untuk memulai ulang.
        """)
    ...
    # Kita menampilkan menu favorit tanpa plus-minus, langsung sebagai output pesanan.
    # Pengguna tinggal Place Order atau Edit Favorite jika ingin mengubah pesanan.
    if st.session_state.edit_fav == False:
        ...
        # Tampilkan menu favorit langsung tanpa number_input (tanpa plus/minus)
        # Asumsikan jumlah setiap item = 1
        total = 0
        menu_array = []
        for m_name, m_price in menus.items():
            # Langsung asumsikan quantity=1
            qty = 1
        ...
        # Tombol Place Order tanpa form karena sudah pasti jumlahnya 1
    ...
    if st.session_state.edit_fav == True:
        ...
        # Di mode edit, gunakan form + number_input agar user dapat ubah jumlah
        with st.form("edit_fav_form"):
            selected_items = {}
            menupick = []
            for i, (m_name, m_price) in enumerate(menus.items()):
                qty = st.number_input(f"{m_name} - Rp.{m_price}", min_value=0, step=1, key=f"edit_{m_name}")
                if qty > 0:
    ...
    with st.form("register_order_form"):
        ...
        menupick = []
        for i, (m_name, m_price) in enumerate(menus.items()):
            qty = st.number_input(f"{m_name} - Rp.{m_price}", min_value=0, step=1, key=f"reg_{m_name}")
            if qty > 0:
                selected_items[m_name] = (m_price, qty)
                menupick.append(i)
