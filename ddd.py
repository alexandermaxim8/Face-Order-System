...
def menu_selector(menu_items):
    ...
    def fdisplay_order_list(order_counts):
        clear_screen()
...
def select_menu():
    ...
    print("menupick:", menupick)
    print("menu:", menu)
    print("selected_order_counts: ", selected_order_counts)
...
#     while True:
#         menupick, menu, order_counts = select_menu()
#         selected_menu = []  # Inisialisasi selected_menu
        
#         for i in order_counts.keys():
#             menu_name = menu[i-1]["fields"]["name"]["stringValue"]
#             menu_price = int(menu[i-1]["fields"]["price"]["integerValue"])
#             quantity = order_counts[i-1]  # Ambil jumlah order berdasarkan indeks
            
#             # Tambahkan ke selected_menu
#             selected_menu.append({
#                 "mapValue": {
#                     "fields": {
#                         "name": {"stringValue": menu_name},
#                         "price": {"integerValue": menu_price},
#                         "quantity": {"integerValue": quantity}
#                     }
#                 }
#             })
            
#             print(f'{menu_name} - Rp.{menu_price} x {quantity} = Rp.{menu_price * quantity}')
        
#         total_price = sum(item["mapValue"]["fields"]["price"]["integerValue"] * 
#                           item["mapValue"]["fields"]["quantity"]["integerValue"]
#                           for item in selected_menu)

#         print(f"Total: {total_price}")
#         print("Your food is being cooked, wait and enjoy!")
...
#     print(selected_menu)
#     fb.log_menu(idToken, email, selected_menu, 0)  # Total price tetap dikalkulasi dalam sistem

def manual():
    text = ["Thank You", "Your Order", "is being Processed", "Order cancelled"]
    while True:
        menupick, menu, order_counts = select_menu()
        selected_menu_dict = {}  # Menggunakan dictionary untuk mencegah duplikasi

        for i in menupick:
            # Dapatkan nama dan harga menu dari menu
            menu_name = menu[i-1]["fields"]["name"]["stringValue"]
            menu_price = int(menu[i-1]["fields"]["price"]["integerValue"])
            
            # Ambil jumlah pesanan dari order_counts berdasarkan nama makanan
            quantity = order_counts.get(menu_name, 0)

            if quantity > 0:  # Tambahkan hanya jika quantity lebih dari 0
                if menu_name not in selected_menu_dict:
                    # Jika belum ada dalam dictionary, tambahkan
                    selected_menu_dict[menu_name] = {
                        "mapValue": {
                            "fields": {
                                "name": {"stringValue": menu_name},
                                "price": {"integerValue": menu_price},
                    ...
                    # Jika sudah ada, perbarui quantity
                    selected_menu_dict[menu_name]["mapValue"]["fields"]["quantity"]["integerValue"] += quantity

                print(f'{menu_name} - Rp.{menu_price} x {quantity} = Rp.{menu_price * quantity}')
        
        # Konversi dictionary ke list
        selected_menu = list(selected_menu_dict.values())
